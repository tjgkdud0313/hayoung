...
function sendMessage(message){
  ...
  fetch("/api/send",{
    ...
    // post로 쏜것 >> post로 주소값을 넘겨주겠다(post 주소를)
      method:"POST",
      headers:{
        // 이게 제이슨 문법을 쓴거다 알려주는거
        //컨테츠 타입이 이거다 
        'Content-Type': 'application/jason',
      },

      // stringify는 객체를 문서화해주는 
      // 요 안에 들어있는 닉이랑 메시지 객체
...
//모든 메시지를 불러오게 해주는 함수
// 서버 새로 열면 기존에 있던 채팅 내용 다 사라지고 빈창 뜨면서 새로 시작하는데
// 실제 카톡같은건 내가 카톡방 열면 기존 내용 다 보존되게 남아있으므로
//그거랑 비슷하게 구현하기 위함 
...
function loadMessages() {
  ...
  //데이터를 불러와라
  fetch("/api/messages",{
    // 이번에는 get으로 쐈으니까 방식은 get 
    method:"GET",
    // 보낼건 없으니까(불러오기만 하니까) 바디는 필요없음 
    headers:{
      // 이게 제이슨 문법을 쓴거다 알려주는거
      'Content-Type': 'application/jason'
    }
